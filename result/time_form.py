...
class SelectTimeWidget(Widget):
    ...
    def render(self, name, value, attrs=None, renderer = None):
        ...
        if self.use_seconds:
            ...
            select_html = Select(choices=second_choices).render(self.second_field % name, second_val)
            output.append(select_html)
    
        if self.twelve_hr:
            meridiem_choices = [('a.m.','a.m.'), ('p.m.','p.m.')]
           
            select_html = Select(choices=meridiem_choices).render(self.meridiem_field % name, meridiem_val)
            output.append(select_html)
